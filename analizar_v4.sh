...
analyze_cobol_file() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        # Detectar variables de nivel (01, 02, ..., 49, 66, 77, 88)
        if [[ $line =~ ^([0-4][0-9]|66|77|88)\ ([A-Z0-9-]+)\.?(.*)$ ]]; then
            level="${BASH_REMATCH[1]}"
            name="${BASH_REMATCH[2]}"

            # Si contiene PIC, es una variable simple
            if [[ ${BASH_REMATCH[3]} =~ PIC\ ([A9XSZVBP\*\$/\.,()+-]+)(\(([0-9]+|\*))\)? ]]; then
                type="${BASH_REMATCH[1]}"
                size="${BASH_REMATCH[3]}"
        ...
        elif [[ -n $current_composite && $line =~ ^([0-4][0-9]|66|77|88)\ ([A-Z0-9-]+)\.?(.*)$ ]]; then
            sub_level="${BASH_REMATCH[1]}"
            sub_name="${BASH_REMATCH[2]}"
            sub_pic="${BASH_REMATCH[3]}"

            sub_entry="{\"name\": \"$sub_name\", \"line\": $current_line, \"level\": \"$sub_level\""

            if [[ $sub_pic =~ PIC\ ([A9XSZVBP\*\$/\.,()+-]+)(\(([0-9]+|\*))\)? ]]; then
                sub_type="${BASH_REMATCH[1]}"
                sub_size="${BASH_REMATCH[3]}"
                sub_entry+=" ,\"type\": \"$sub_type\", \"size\": \"$sub_size\""
            fi

            sub_entry+="}"

            # Agregar subvariable al compuesto actual
            composite_vars[$current_composite]=$(echo "${composite_vars[$current_composite]}" | jq ".subvars += [$sub_entry]")
        fi
    done < "$cobol_file"
